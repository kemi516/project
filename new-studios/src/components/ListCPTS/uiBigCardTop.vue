<template>
  <div class="ui-big-cardTop">
    <div class="content">
      <div class="video" @click.stop="videoDetails" ref="videoDiv">
        <!-- 图片 -->
        ...
        <div class="number" v-show="isPlay">
          ...
          <span>{{
          bigCard.resource.count.count_comment >= 10000
          ? bigCard.resource.count.count_comment >= 100000000
          ? (bigCard.resource.count.count_comment / 100000000).toFixed(
          1
          ) + "e"
          : (bigCard.resource.count.count_comment / 10000).toFixed(1) +
          "w"
          : bigCard.resource.count.count_comment >= 1000
          ? (bigCard.resource.count.count_comment / 1000).toFixed(1) + "k"
          : bigCard.resource.count.count_comment
          }}
            评论 &nbsp;&nbsp;|&nbsp;&nbsp;</span>
          <span>{{
          bigCard.resource.count.count_view >= 10000
          ? bigCard.resource.count.count_view >= 100000000
          ? (bigCard.resource.count.count_view / 100000000).toFixed(1) +
          "e"
          : (bigCard.resource.count.count_view / 10000).toFixed(1) + "w"
          : bigCard.resource.count.count_view >= 1000
          ? (bigCard.resource.count.count_view / 1000).toFixed(1) + "k"
          : bigCard.resource.count.count_view
          }}
            观看&nbsp;&nbsp;|&nbsp;&nbsp;</span>
          <span>
            {{
        ...
        <video v-if="bigVideo?.cover" :muted="!mutedState" ref="video" class="vvv" :autoplay="!isPlay"
          ...
          " @play="isPlay = false" @playing="playingState" @waiting="ddd" @abort="isLose" @canplay="canBoFan()"
          preload="metadata"></video>
      </div>
      <div class="allView" v-if="bigCard?.attributes?.tags">
        <div>
          <figure v-lazy:background-image="'image/xpcAll/2v.png'"></figure>
          <span>{{ bigCard.attributes.tags[0].title }}</span>
        </div>
      </div>
      <div class="title">
        ...
        <div class="oper">
          ...
          ">
            ...
            bigCard.resource.count.count_collect >= 10000
            ? bigCard.resource.count.count_collect >= 100000000
            ? (bigCard.resource.count.count_collect / 100000000).toFixed(
            1
            ) + "e"
            : (bigCard.resource.count.count_collect / 10000).toFixed(1) +
            "w"
            : bigCard.resource.count.count_collect >= 1000
            ? (bigCard.resource.count.count_collect / 1000).toFixed(1) + "k"
            : bigCard.resource.count.count_collect
            }}</span>
          </figure>
          <figure @click.stop="
          ...
          ">
            ...
            bigCard.resource.count.count_like >= 10000
            ? bigCard.resource.count.count_like >= 100000000
            ? (bigCard.resource.count.count_like / 100000000).toFixed(1) +
            "e"
            : (bigCard.resource.count.count_like / 10000).toFixed(1) + "w"
            : bigCard.resource.count.count_like >= 1000
            ? (bigCard.resource.count.count_like / 1000).toFixed(1) + "k"
            : bigCard.resource.count.count_like
            }}</span>
          </figure>
          <figure @click.stop="$store.commit('shareStateUpdate')">
            <img v-lazy="'image/xpcAll/u5.png'" alt="" />
          </figure>
        </div>
      </div>
      <ul class="author" v-if="bottomUser">
        ...
        <p v-if="bottomUser.length > 2">和其他{{ bottomUser.length - 1 }}人</p>
        <span v-if="bottomUser.length > 2 || bottomUser.length == 1" class="is-active" v-show="!atten"
          @click.stop="atten = !atten">关注</span>
        <span v-if="bottomUser.length > 2 || bottomUser.length == 1" class="is-active" v-show="atten"
          :style="{ color: atten && '#999' }" @click.stop="atten = !atten">已关注</span>
      </ul>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      videoSrc: null,
      bigCard: this.uiBigCardTop?.children[0]?.model,
      bigVideo: null,
      isPlay: true,
      mutedState: false,
      currentTime: 0,
  ...
  created() {
    ...
    this.bottomUser = [
      ...
      ...(this.bigCard.resource.team
        ? this.bigCard.resource.team.filter((e) => e.userinfo.vip_status == 1)
        : []),
    ];
  },
  computed: {
  ...
  methods: {
    scrollAddEvent() {
      try {
        if (this.tabsActiveIndex == this.tabsActive) {
          ...
          this.time = setTimeout(() => {
            ...
            if (cha > 0 && cha < this.$refs.videoDiv?.offsetHeight) {
        ...
        console.log("bigcard又报错啦", error);
      }
      // try {
      //   if (this.tabsActive == this.tabsActiveIndex) {
      //     clearTimeout(this.time);
      //     this.time = setTimeout(() => {
      //       let innerHeight = window.innerHeight - 95 - this.$refs.videoDiv?.offsetHeight;
      //       let cha = this.$refs.videoDiv?.offsetTop - window.scrollY;
      //       if (!this.anewState) {
      //         if (cha > 0 && cha < innerHeight) {
  ...
  watch: {
    tabsActive(n) {
      ...
      if (n == this.tabsActiveIndex && this.$route.path == "/") {
        clearTimeout(this.time);
        this.time = setTimeout(() => {
          let innerHeight =
            window.innerHeight - 95 - this.$refs.videoDiv?.clientHeight;
